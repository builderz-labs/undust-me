...
function SwapModal({ isSwapModalOpen, setIsSwapModalOpen, rentBack }: any) {
    ...
    return (
        <>
            {isSwapModalOpen && <div className='w-full h-screen absolute inset-0 z-40 bg-black bg-opacity-20 backdrop-blur-sm flex items-center justify-center'>
                    ...
                    <div className="w-full text-center px-4 flex items-center justify-center flex-col">
                        ...
                        <div className="w-full my-4 px-4 flex items-center justify-center mt-10 gap-4">
                            {['x1', 'x2', 'x5', 'x10'].map((multiplier, index) => (
                                <div
                                    key={index}
                                    onClick={() => handleMultiplierClick(multiplier === 'x1' ? 1 : parseInt(multiplier.slice(1)))}
                                    className={`w-12 font-bold h-12 flex bg-undust-green ${selectedMultiplier === (multiplier === 'x1' ? 1 : parseInt(multiplier.slice(1))) ? 'bg-opacity-80' : 'bg-opacity-10'} border border-white border-opacity-20 text-white items-center justify-center rounded-lg cursor-pointer hover:bg-opacity-60`}
                                >
                                    {multiplier}
                                </div>
                            ))}
                        ...
                        {selectedMultiplier && (
                            ...
                            >
                                Swap Now for {rentBack * selectedMultiplier}
                            </button>
                        )}
                    </div>
                </div>
